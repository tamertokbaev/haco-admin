...
const ContestAddUpdateModal: FC<Props> = ({isOpen, handleClose, contestPrize, contestId, onSuccessModify}) => {
  ...
  const {
    ...
    formState: {errors},
    // @ts-ignore
  } = useForm<ContestPrizeCreate>({resolver: yupResolver(ContestValidationSchema), defaultValues: {...contestPrize}})
  ...
  const submitForm = (data: ContestPrizeCreate) => {
    ...
    let promiseToExecute = contestPrize
      ? BackendService.updateContestPrize({...formData, contest_prize_id: contestPrize.contest_prize_id})
      : BackendService.createContestPrize(formData)
